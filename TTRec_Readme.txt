...
■使い方
TVTestのPluginsフォルダにTTRec.tvtpを入れ、TVTestを起動して右クリックメニューか
らTTRecを有効化してください。プラグイン設定は 右クリックメニュー→設定→プラグイ
ン→TTRec から行います。また、EPG番組表→右クリックメニュー から予約設定・キーワ
ード検索予約登録(クエリ設定)を行います。
当プラグイン使用の際はTVTestの複数起動を禁止してください(設定→一般→複数起動を
禁止する)。また、録画停止の確認をとることを勧めます(設定→録画→録画停止は確認を
とる)。

■プラグイン設定
[使用するBonDriver]
  タスクスケジューラからの起動やチャンネル変更時に、指定のドライバを使います。
[TOT時刻補正]
...
[複数のTVTestで使いたい]
  タスクスケジューラの登録名が被らないよう、プラグインのファイル名を変えてくださ
  い(たとえば、"TTRec1.tvtp"と"TTRec2.tvtp"など)。
...
■その他
当プラグイン、機能的にはTvRockやEpgDataCap_Bonと被ります。当方のPC環境がアレでこ
れらの常駐が厳しかったため作りました。自分が使う機能だけ実装しているため今のとこ
ろ機能は限定的で貧弱です。
