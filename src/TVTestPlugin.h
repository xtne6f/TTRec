...
// pNumServices が NULL でない場合は、試聴可能なサービスの数が返されます。
...
// pszPath を NULL で呼べば長さだけを取得できます。
...
struct EpgEventInfo
{
	...
	LPCWSTR pszEventName;               // イベント名(無い場合は NULL)
	LPCWSTR pszEventText;               // テキスト(無い場合は NULL)
	LPCWSTR pszEventExtendedText;       // 拡張テキスト(無い場合は NULL)
	EpgEventVideoInfo **VideoList;      // 映像の情報のリスト(無い場合は NULL)
	EpgEventAudioInfo **AudioList;      // 音声の情報のリスト(無い場合は NULL)
	EpgEventContentInfo *ContentList;   // ジャンルの情報(無い場合は NULL)
	EpgEventGroupInfo **EventGroupList; // イベントグループ情報(無い場合は NULL)
};
